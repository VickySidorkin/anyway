...
def main(dest_folder, download_images):
    ...
    print(colored('Got all road page links, starting to process road links', 'green'))
    ...
    for link_suffix in all_road_page_links:
        ...
        if processed % 10 == 0:
            print(colored(f"processed: {processed_percent}%", 'green'))
    ...
    print(colored('Saving road SVG links to file...', 'green'))
...
def find_all_road_svg_links(link_suffix, svg_extension_pattern):
    ...
    if svg_extension_pattern.search(link_suffix) is not None:
        ...
        try:
            ...
            print(f"could not get page - {WIKIPEDIA_RELEVANT_DOMAIN + link_suffix}. skipping...")
            return []
...
def get_svg_file_from_svg_page(road_svg_link_suffix, svg_extension_pattern):
    ...
    try:
        ...
        print(f"could not get image file - {WIKIPEDIA_RELEVANT_DOMAIN + road_svg_link_suffix}. skipping...")
        return None
...
def update_all_road_page_links(all_road_links):
    ...
    try:
        ...
        print(f"could not get main page for all roads - {WIKIPEDIA_ISRAELI_ROADS_LINK}")
        return
